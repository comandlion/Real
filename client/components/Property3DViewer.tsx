...
import { OrbitControls, Environment, PerspectiveCamera, Text, Box, Sphere, Plane } from '@react-three/drei';
...
// 3D House Component
function House({ dimensions = { width: 10, length: 12, height: 8 } }: { dimensions?: any }) {
  const houseRef = useRef<THREE.Group>(null);
  
  return (
    <group ref={houseRef} position={[0, dimensions.height / 2, 0]}>
      {/* Main house structure */}
      <Box args={[dimensions.width, dimensions.height, dimensions.length]} position={[0, 0, 0]}>
        <meshStandardMaterial color="#e5e7eb" />
      </Box>
      
      {/* Roof */}
      <Box args={[dimensions.width + 1, 2, dimensions.length + 1]} position={[0, dimensions.height / 2 + 1, 0]}>
        <meshStandardMaterial color="#7c3aed" />
      </Box>
      
      {/* Door */}
      <Box args={[1.5, 3, 0.2]} position={[0, -dimensions.height / 2 + 1.5, dimensions.length / 2 + 0.1]}>
      ...
      {/* Garage */}
      <Box args={[4, 3, 6]} position={[dimensions.width / 2 + 2, -dimensions.height / 2 + 1.5, 0]}>
...
function LandPlot({ dimensions = { width: 50, length: 60 } }: { dimensions?: any }) {
  const landRef = useRef<THREE.Group>(null);
  
  return (
    <group ref={landRef}>
      {/* Land base */}
      <Plane args={[dimensions.width, dimensions.length]} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]}>
      ...
      <group>
        {/* Boundary markers */}
        <Sphere args={[0.5]} position={[dimensions.width / 2, 0.5, dimensions.length / 2]}>
...
export function Property3DViewer({
  ...
  const [currentView, setCurrentView] = useState<'exterior' | 'interior' | 'aerial'>('exterior');
  ...
  return (
    <Card className={`border-0 shadow-lg overflow-hidden ${className} ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
      <CardContent className="p-0 relative">
        <div className={`relative ${isFullscreen ? 'h-screen' : 'h-96'} bg-gradient-to-b from-sky-200 to-green-100`}>
          <Canvas>
            <Suspense fallback={<LoadingModel />}>
              <PerspectiveCamera 
                makeDefault 
                position={cameraPositions[currentView] as [number, number, number]} 
                fov={60}
              />
              ...
              <motion.div key={view} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button
                  ...
                  className={`bg-white/90 backdrop-blur-sm border-gray-200 hover:bg-white shadow-lg capitalize ${
                    currentView === view ? 'bg-luxury-blue text-white hover:bg-blue-600' : ''
                  }`}
                >
          ...
          {showInfo && (
            ...
            >
              <div className="flex items-center space-x-2 mb-2">
                <Badge className={`${propertyType === 'land' ? 'bg-emerald-500' : 'bg-luxury-blue'} text-white`}>
                  {propertyType === 'land' ? <TreePine className="h-3 w-3 mr-1" /> : <Home className="h-3 w-3 mr-1" />}
              ...
              {dimensions && (
                <div className="text-sm text-gray-600 mb-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {dimensions.height && <div>Height: {dimensions.height}ft</div>}
                  </div>
                </div>
              )}
